...
def writer_worker(queue, output_path, schema, mode):
    ...
    while True:
        ...
        try:
            ...
            events = process_output_format(
                peaks, chunk_borders, read_ids, mode, signal_chunks
            )
            ...
            print(
                f"[writer_worker] skipping batch due to error: {type(e).__name__}: {e}"
            )
    writer.close()
...
def predict(model_path, devices, pod5_rids_pairs, bs, tgt_file, workers, mode):
    ...
    for pod5_path, rids in pod5_rids_pairs:
        ...
        for chunks, chunk_borders, read_ids, signal_chunks in tqdm.tqdm(
            get_raw_batch3(reader, rids, bs)
        ):
            ...
            signal = torch.stack(
                [torch_chunks, diff_gpu, gpu_w_means, gpu_w_stds, tstat1_gpu], dim=1
            )
...
def main(args):
    ...
    predict(
        ...
        pod5_readid_pairs,
        args.bs,
        f"{args.tgt_dir}/{args.abbrev}_events",
        1,
        args.mode,
    )
    full_end = time.time()
